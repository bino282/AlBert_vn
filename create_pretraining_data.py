...
class TrainingInstance(object):
  ...
  def __str__(self):
    s = ""
    s += "tokens: %s\n" % (" ".join(
        [tokenization_vn.printable_text(x) for x in self.tokens]))
    ...
    s += "masked_lm_labels: %s\n" % (" ".join(
        [tokenization_vn.printable_text(x) for x in self.masked_lm_labels]))
    s += "\n"
    return s
  ...
  for (inst_index, instance) in enumerate(instances):
    ...
    if inst_index < 20:
      ...
      tf.logging.info("tokens: %s" % " ".join(
          [tokenization_vn.printable_text(x) for x in instance.tokens]))
    ...
    with tf.gfile.GFile(input_file, "r") as reader:
      while True:
        ...
        if not FLAGS.spm_model_file:
          line = tokenization_vn.convert_to_unicode(line)
        if not line:
          break
        if FLAGS.spm_model_file:
          line = tokenization_vn.preprocess_text(line, lower=FLAGS.do_lower_case)
        else:
          line = line.strip()
